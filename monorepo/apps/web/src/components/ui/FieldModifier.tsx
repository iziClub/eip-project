...
// Types pour chaque type de champ
type FieldValueType = {
  text: string;
  textarea: string;
  file: File;
  ...
  onSave,
    ...
    const handleOpen = () => {
        ...
        const editor: Record<EditableFieldType, JSX.Element> = {
            ...
            textarea: (
                <div>Textarea editor not implemented yet.</div>
            ),
            file: (
                <FileUpload
                    multiple={false}        
                    onFilesChange={(files) => {
                        // Prendre le premier fichier du tableau ou null si vide
    ...
    // Fonction helper pour afficher la valeur
    const displayValue = () => {
        if (type === "file") {
                ...
                { (type === "file") &&
                    <div className="w-full bg-grey-1 rounded-[10px] border border-grey-2 px-4 flex justify-between">
                        <div className="flex flex-col gap-1">
                            ...
                            :
                            <span className="text-sm font-semibold text-grey-3 py-3">Aucun fichier</span>
                            }
                        </div>
