...
export function FieldModifier<T extends EditableFieldType>({
  ...
  onSave,
    ...
    return (
        <div className="">
            <Label className="text-base font-semibold text-black mb-2">{label}</Label>
            <div className="flex flex-row w-full justify-between gap-4 mt-2">
                ...
                {(type === "file") && (
                    <div className="w-full bg-grey-1 rounded-[10px] border border-grey-2 px-4 flex justify-between">
                        <div className="flex flex-col gap-1">
                            {file?.name ? (
                                <>
                                    <span className="text-sm font-semibold text-black pt-3 truncate">
                                        {displayValue()}
                                    </span>
                                    <span className="text-xs text-grey-3 pb-3">
                                        {displayFileSize()} Mo
                                    </span>
                                </>
                            ) : (
                                <span className="text-sm font-semibold text-grey-3 py-4">
                                    Aucun fichier
                                </span>
                            )}
                        </div>
                        ...
                        <button>
                            <svg 
                                className={`${file?.name ? '' : 'hidden'} fill-black hover:fill-red-primary transition-colors cursor-pointer`} 
                                width="18" 
                                height="20" 
                                viewBox="0 0 18 20" 
                                fill="none" 
                ...
                <Button onClick={handleOpen} variant="modifier" size="icon-full">
                    <svg 
                        className="fill-blue-secondary group-hover/button:fill-white" 
                        width="19" 
                        height="19" 
                        viewBox="0 0 19 19" 
                        fill="none" 
